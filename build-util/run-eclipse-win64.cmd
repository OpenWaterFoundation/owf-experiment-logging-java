...
title Eclipse configured for Java Logging Experiment
rem The -data option must come before -vm, etc.
rem Can omit -data the first time if workspace folder does not exist.
rem %eclipseExe% -data %workspaceFolder% -vm %javawExe% -vmargs -Xmx1024M
rem Don't set the workspace since different users may configure differently.
